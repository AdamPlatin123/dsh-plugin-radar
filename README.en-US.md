...
This README is now bilingual (one Chinese line, one English line) in the main [README.md](README.md).
本 README 已合并为双语版（一行中文一行英文），见主 [README.md](README.md)。
<p align="center">
...
**What is this?** DeepSeek Harness (DSH) is an open-source coding agent where everything is a plugin. This repo is a **radar** that automatically tracks its plugin ecosystem — **1304 plugin repos indexed** (manifest-level classification, v2 engine), **13336 runtime-tested on the k8s track**.
...
> Data as of snapshot `20260911T154501Z` (2026-09-11 23:45:03 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 19988 · age 111m"]
    ...
    B1 -->|"plugins 1304"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 0)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 13336"}
    D1 -->|"8594 / 2755"| E1["aggregate + README stats"]
    D1 -->|"1987 env retries"| C1
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260911T154501Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260911T154501Z（2026-09-11 23:45 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 8594 可用 · 2755 不兼容 · 1987 待定（共 13336 个，k8s agent 口径）|
