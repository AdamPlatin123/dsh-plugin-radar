...
> Data as of snapshot `20260908T091501Z` (2026-09-08 17:15:03 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 19096 · age 3m"]
    ...
    B1 -->|"plugins 1178"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 0)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 13334"}
    D1 -->|"8919 / 2799"| E1["aggregate + README stats"]
    D1 -->|"1616 env retries"| C1
...
> 人工策展 56 款插件，按 11 类分组、类内按星标排序；星标每 6 小时自动刷新（成员调整请提 PR 修改 data/awesome-50.json）。数据截至 2026-09-08 19:27（UTC+8）。
...
- <img src="assets/tile-adapt.svg" alt="需适配" width="122" height="20"> **[dsh-desktop](https://github.com/anywhere-labs/dsh-desktop)** · 24436★ — 生态最高星桌面客户端（21.5k★，原 deepseek-harness-desktop 再改名）：万物皆插件、桌面本身也是插件（雷达重测中；rc.8 源码路径实测 ✅）
...
> 人工策展 16 个整合包：内测成员作品置顶，其下按预设套件 / 能力合集 / 发行版 / 配方管理器四形态分组，类内按星标排序；星标每 6 小时自动刷新（成员调整请提 PR 修改 data/bundles.json）。数据截至 2026-09-08 19:27（UTC+8）。
...
- **🔌 Web UI 增强**（2778）— OK 1343 · incompatible 404 · pending 254 · untested 17 · watching 760 — [details](catalog/all/Web%20UI%20增强.md)
- **💻 编码开发**（2154）— OK 915 · incompatible 365 · pending 192 · untested 19 · watching 663 — [details](catalog/all/编码开发.md)
- **🤖 Agent 能力**（2254）— OK 817 · incompatible 275 · pending 166 · untested 11 · watching 985 — [details](catalog/all/Agent%20能力.md)
- **📡 消息通讯**（722）— OK 260 · incompatible 115 · pending 58 · untested 4 · watching 285 — [details](catalog/all/消息通讯.md)
- **🗂 文件数据**（675）— OK 268 · incompatible 96 · pending 56 · untested 9 · watching 246 — [details](catalog/all/文件数据.md)
- **🎮 娱乐生活**（455）— OK 174 · incompatible 35 · pending 31 · untested 0 · watching 215 — [details](catalog/all/娱乐生活.md)
- **🛠 基建部署**（1361）— OK 441 · incompatible 133 · pending 128 · untested 4 · watching 655 — [details](catalog/all/基建部署.md)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260908T091501Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260908T091501Z（2026-09-08 17:15 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 8919 可用 · 2799 不兼容 · 1616 待定（共 13334 个，k8s agent 口径）|
