...
> Data as of snapshot `20260907T220001Z` (2026-09-08 06:00:03 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 18991 · age 48m"]
    ...
    B1 -->|"plugins 1154"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 0)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 13334"}
    D1 -->|"8932 / 2764"| E1["aggregate + README stats"]
...
- **🔌 Web UI 增强**（2776）— OK 1338 · incompatible 400 · pending 262 · untested 17 · watching 759 — [details](catalog/all/Web%20UI%20增强.md)
- **💻 编码开发**（2154）— OK 918 · incompatible 360 · pending 194 · untested 19 · watching 663 — [details](catalog/all/编码开发.md)
...
- **🛠 基建部署**（1359）— OK 442 · incompatible 134 · pending 124 · untested 4 · watching 655 — [details](catalog/all/基建部署.md)
- **📚 学习研究**（156）— OK 37 · incompatible 8 · pending 8 · untested 2 · watching 101 — [details](catalog/all/学习研究.md)
- **❓ 其他**（7205）— OK 1833 · incompatible 371 · pending 254 · untested 21 · watching 4726 — [details](catalog/all/其他.md)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260907T220001Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260907T220001Z（2026-09-08 06:00 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 8932 可用 · 2764 不兼容 · 1638 待定（共 13334 个，k8s agent 口径）|
